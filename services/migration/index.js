...
let posts = require(path.resolve('data/posts_parsed.json')).filter(post => post.status === 'active')
...
const authors = [ 
  ...
  // use this if not found any author
  { entryId: '1F3j0fqFp2Ek0Q8KgsuKac', author_id: 99 }
]

const SPACE_ID = process.env.SPACE_ID

const client = contentful.createClient({
  accessToken: process.env.MANAGING_ACCESS_TOKEN
})

const removeEntries = space => {
  space.getEntries({
    'content_type': 'post',
    'select': 'sys.id'
  })
...
const createEntriesInBatches = (posts, space) => {
  ...
  logger.log({
    level: 'info',
    message: 'START: create entries in batches'
  });
  ...
  function makeCallToCrateEntries() {
    if (i >= postsLimit) {
      logger.log({
        level: 'info',
        message: 'END: create entries in batches'
      });
    ...
    logger.log({
      level: 'info',
      message: `sent batch nr: ${i}/${postsLimit}`
    });

    const postsBatch = slice(posts, i)

    postsBatch.forEach(post => space.createEntry('post', post))
